    ...
    public AStarSolver(AStarGraph<Vertex> input, Vertex start, Vertex end, double timeout) {
        ...
        while (minPQ.size() > 0) {
            ...
            for (WeightedEdge<Vertex> edge: input.neighbors(curr)) {
                if (!distTo.containsKey(edge.to()) || distToCurr + edge.weight() < distTo.get(edge.to())) {
                    distTo.put(edge.to(), distToCurr + edge.weight());
                    edgeTo.put(edge.to(), curr);

                    if (minPQ.contains(edge.to())) {
                        minPQ.changePriority(edge.to(), distTo.get(edge.to()) + input.estimatedDistanceToGoal(edge.to(), end));
                    } else {
                        minPQ.add(edge.to(), distTo.get(edge.to()) + input.estimatedDistanceToGoal(edge.to(), end));
                    }
                }
            }
        }
